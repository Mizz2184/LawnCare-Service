...
type Props = {
  ...
  onViewServiceDetail?: (slug: string) => void
}

function getSlugForService(name: string): string {
  const n = name.toLowerCase()
  if (n.includes('mow')) return 'weekly-lawn-mowing'
  if (n.includes('hedge') || n.includes('shrub') || n.includes('trim')) return 'hedge-shrub-trimming'
  if (n.includes('cleanup') || n.includes('clean-up') || n.includes('season')) return 'seasonal-yard-cleanup'
  if (n.includes('fertil') || n.includes('treat') || n.includes('feed')) return 'fertilization-visit'
  if (n.includes('consult') || n.includes('plan') || n.includes('design')) return 'landscape-consultation'
  return 'full-lawn-maintenance-visit'
}

export default function Services({ onSelectService, onViewServiceDetail }: Props) {
            ...
            {services.map((service, idx) => (
              <ServiceCard
                ...
                onViewDetail={() => {
                  ...
                  if (onViewServiceDetail) onViewServiceDetail(slug)
                  else window.location.hash = `#/services/${slug}`
                }}
                featured={idx === 1}
              />
            ))}
    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden cursor-pointer" onClick={onViewDetail}>
        ...
        <div className="absolute top-3 left-3 flex gap-2">
          ...
          {featured && (
            <span className="badge bg-forest-900/90 text-cream-50 backdrop-blur">Popular</span>
          )}
        </div>
      ...
      <div className="p-6 flex flex-col flex-1">
        <h3
          ...
          className="font-display text-xl font-semibold text-forest-900 leading-snug cursor-pointer hover:text-forest-700 transition-colors"
        >
          {service.name}
        </h3>
        {service.description && (
        ...
        <div className="mt-auto pt-5 flex items-center justify-between gap-2 border-t border-ink-100/60">
          <a
            href={`#/services/${slug}`}
            onClick={(e) => {
              e.preventDefault()
          ...
          <button
            onClick={onBook}
            className="inline-flex items-center gap-1.5 text-xs font-semibold bg-forest-900 hover:bg-forest-800 text-cream-50 px-3.5 py-2 rounded-full transition-colors"
          >
            <span>Book Now</span>
